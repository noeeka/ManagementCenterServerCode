...
$.ajax({
  url: _ajaxLink + "?c=Configuration&m=getCityList",
  type: "GET",
  dataType: "json",
  async: false,
    ...
    Plugin.prototype = {
        ...
        createProvince: function(e) {
            ...
            for (var i = 0; i < area.length; i++) {
                if (selPro == area[i].name) {
                    proStr += '<li id="'+ area[i].id +'" class="ulli pick-selectedLi" style="background:' + this.config.hoverColor + ';color:#fff;font-weight:bold;">' + area[i].name + '</li>'
                } else {
                    proStr += '<li id="'+ area[i].id +'" class="ulli">' + area[i].name + '</li>'
                }
            }
            ...
            if (cityJson.length != 0) {
                for (var j = 0; j < cityJson.length; j++) {
                    if (selCity == cityJson[j].name) {
                        cityStr += '<li id="'+ cityJson[j].id +'" class="ulli pick-selectedLi" style="background:' + this.config.hoverColor + ';color:#fff;font-weight:bold;">' + cityJson[j].name + '</li>'
                    } else {
                        cityStr += '<li id="'+ cityJson[j].id +'" class="ulli">' + cityJson[j].name + '</li>'
                    }
                }
        ...
        setVal: function(_this, aim, s) {
            ...
            if (s == "pro") {
                ...
                _this.ele.siblings('.button--orange').attr('data-id', cityId)
                _this.ele.find(".pick-county").html("请选择县");
